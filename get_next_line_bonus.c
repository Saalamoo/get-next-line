...
/*   Updated: 2023/01/31 14:00:29 by saalamoo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*tmp;
	static char	*b3d[OPEN_MAX];
	int			j;

	tmp = NULL;
	if (!b3d[fd])
		tmp = ft_strdup("");
	tmp = new_line_read_everything(fd, buf, tmp, b3d[fd]);
	j = -1;
	while (tmp[++j])
	{
		if (tmp[j] == '\n')
	...
	b3d[fd] = ft_substr(tmp, j + 1, ft_strlen(tmp) - j - 1);
	gnl = ft_substr(tmp, 0, j + 1);
	if (tmp != NULL)
		free(tmp);
	if (b3d[fd] && ft_strlen(gnl) == 0)
	{
		free(b3d[fd]);
		b3d[fd] = NULL;
	}
